...
class JumlahWPSuperAdminPresenter (
    val context: Context?,
    val client: APIClient,
    val superAdminView: JumlahWPSuperAdminView
): AnkoLogger {
...
//                        superAdminView.hideLoading()
    ...
    fun getJumlahWP(kec: String) {
        superAdminView.showLoading()
        client.jumlahWPSuperAdmin(kec).enqueue(object : Callback<ResponseJSON> {
            override fun onFailure(call: Call<ResponseJSON>, t: Throwable) {
                info("${context?.getString(R.string.title_failure)} ${t.toString()}")
                superAdminView.hideLoading()
            }
    ...
    fun getJumlahWPBayar(kec: String) {
        superAdminView.showLoading()
        client.jumlahWPBayarSuperAdmin(kec).enqueue(object : Callback<ResponseJSON> {
            override fun onFailure(call: Call<ResponseJSON>, t: Throwable) {
                info("${context?.getString(R.string.title_failure)} ${t.toString()}")
                superAdminView.hideLoading()
            }
            ...
            override fun onResponse(call: Call<ResponseJSON>, response: Response<ResponseJSON>) {
                if (response.isSuccessful) {
                    if (response.body() != null) {
                        ...
                        superAdminView.getJumlahWPKecamatan(totalSPPT, totaSPPTBayar)
                        superAdminView.hideLoading()
                    }
                }
            }
